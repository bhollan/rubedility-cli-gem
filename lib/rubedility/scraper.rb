...
class Scraper
  ...
  def self.scrape_index_page(index_url)
    ...
    index.css("div.lessons_list a").each do |lesson|
      ...
      #lesson_url = "".concat(index_url).concat(lesson.attr("href").delete "/programmers/lessons/")
  ...
  def self.scrape_lesson_page(lesson_url)
    begin
      ...
      lesson.css("div.task-box").each do |task_row|
        ...
        difficulty = task_row.css("div.difficulty").text
        tagline = task_row.css("div.synopsis").text
        puts [name, task_url, difficulty, tagline]
      end
